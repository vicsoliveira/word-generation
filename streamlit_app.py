...
if uploaded_excel is not None and uploaded_word is not None:
    ...
    data_mapping = {
        ...
        "{{PIB}}": df.iloc[26, df.columns.get_loc("Unnamed: 2")],  # Adjust based on actual data
        "{{Percentual da agricultura}}": df.iloc[27, df.columns.get_loc("Unnamed: 2")],  # Adjust based on actual data
        "{{Valor Adicionado Bruto Agropecuária}}": df.iloc[34, df.columns.get_loc("Unnamed: 2")],  # Adjust based on actual data
        "{{Valor Adicionado Bruto Indústria}}": df.iloc[35, df.columns.get_loc("Unnamed: 2")],  # Adjust based on actual data
        "{{Valor Adicionado Bruto Serviços}}": df.iloc[36, df.columns.get_loc("Unnamed: 2")],  # Adjust based on actual data
        "{{Valor Adicionado Bruto Administração Pública}}": df.iloc[37, df.columns.get_loc("Unnamed: 2")]  # Adjust based on actual data
    }
