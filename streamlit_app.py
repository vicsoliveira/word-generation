...
if uploaded_excel is not None and uploaded_word is not None:
    ...
    data_mapping = {
        ...
        "{{Valor Adicionado Bruto Indústria}}": df.iloc[26, df.columns.get_loc("Unnamed: 1")],
        "{{Valor Adicionado Bruto Serviços}}": df.iloc[27, df.columns.get_loc("Unnamed: 1")],
        "{{Valor Adicionado Bruto Administração Pública}}": df.iloc[28, df.columns.get_loc("Unnamed: 1")]
    }
